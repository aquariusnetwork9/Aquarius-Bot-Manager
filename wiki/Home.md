...
- **Notifications** — push alerts via **[ntfy](https://ntfy.sh)** (no account needed) for scheduled jobs, watchdog restarts, and bot online/offline, plus 22 per-bot AquariusProxy events (combat, queue, proxy security, and more). A **🔔 My Notifications** page lets each person pick exactly what *they* want per bot, on one personal topic, server-side filtered — independent of the shared fleet topic. Self-hostable to remove ntfy.sh's rate limit. Optional Apprise fanout (Telegram/Pushover/Slack/etc) or a legacy Discord webhook. See **[[Notifications]]**.
