...
- **Box switcher** — pick which box you're viewing right from the dashboard chrome: the sidebar box chip (Graphite theme) or the pill next to the brand (Client theme), plus the legacy sticky bar shown on proxied pages. Click it for a dropdown listing the current box and every linked box, each with a live online/offline dot — pick one to reverse-proxy its **full native dashboard** into the same tab (console, config, files, proxies, limits — everything), no extra tunnel or browser tab. Picking an **offline** box reconnects its tunnel first and only switches once that succeeds, instead of dropping you on a dead "unreachable" page. A **🖥 Fleet…** entry at the bottom of the dropdown opens the Boxes panel below. Owner/admin only — hidden for guests and non-admin named users, same as the rest of the fleet tools.
- **Fleet view** (the Boxes panel) — every box at a glance: reachable, bots running, host load/mem. Every row has a real working action: **Open** for a reachable box (the controller's own row included — you're never stuck without a way back once you've switched to another box), **↻ Reconnect** for an offline one. Fleet-wide **Start / Restart / Stop all**, and **Update all nodes** (pushes `selfupdate` to each node; the controller updates itself with its own button).
