...
- **Fix: the box switcher wasn't actually hidden from guests/non-admin users in the Graphite sidebar.** The chip and the "🖥 Boxes" nav link (and the rest of the Infrastructure/System nav group) rendered for everyone — clicking them just hit a wall of 403s, since the server already blocked the underlying requests, but the controls themselves shouldn't have been visible at all. They're now `owner-only`-gated in the sidebar the same way the Client theme's equivalents already were.
- **Fix: the manager's self-update could silently fail to restart.** `abm selfupdate` pulled the new code and reported success even when the box's passwordless-sudo restart step wasn't actually configured — the process kept running the old code with no indication anything was wrong. It now checks sudo access first and reports a clear error instead of a false "restarted".
