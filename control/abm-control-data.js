...
{ id:'elytra', name:'Elytra Autopilot', raw:'ElytraPilot', icon:'🪂', cat:'control',
  ...
  groups:[
    ...
    {title:'Pitstop & goal stop  ·  logout / set-spawn', open:false, rows:[T('Pitstop — log out on arrival','toggle',false),T('Pitstop — set a spawn point','toggle',false),T('Pitstop X','num',0,{id:'pitstopX'}),T('Pitstop Z','num',0,{id:'pitstopZ'}),T('Pitstop is a nether coordinate','toggle',false),T('Reached within','num',24,{unit:'blocks'}),T('Relogin after','num',30,{unit:'min'}),T('Relogin even if spawn-set failed','toggle',true),T('Goal — log out at the destination','toggle',false),T('Goal — set a spawn point','toggle',false),T('Goal — relogin after','num',0,{unit:'min'}),T('Goal — relogin even if spawn-set failed','toggle',true)]},
